...
Flutter **Android** app: a multi-provider (xAI Grok + OpenAI) chat **and image**
bot with a shared credit platform, Google login, and Google Play in-app purchase
top-up. Internal-testing on Play.

> **STATUS: HELD 2026-06-24** at Play internal versionCode 25 (`1.3.13+25`).
> Billing-abuse hardening shipped; trial credits set to **0** (top-up required).
> Resume guide + done/gaps list: bottom of `README.md`.
...
| Version (pubspec) | `1.3.13+25` (live on internal) — bump `+N` every Play upload (Play rejects dup versionCode) |
| GitHub | https://github.com/gunug/chat_grok |
| Supabase project (SHARED with android-planlet) | `oerrgsanrnelhvgikgkv` |
| Providers | xAI Grok + OpenAI (selectable via `cg_models` / `cg_image_models`), streaming chat |

## Architecture
```
Flutter app ──JWT──► Supabase Edge Functions ──► xAI / OpenAI / Google Play
  chat:            supabase/functions/chat            (stream + credit gate/deduct)
  image:           supabase/functions/image           (compose + render + gate/deduct)
...
- `chat`: auth (getUser) → `app_register_service`(**trial 0** now — was 300/100) →
  base gate `balance_credits ≥ MIN_BALANCE_CREDITS` (1) → **pre-auth gate**: estimate
  input tokens (`chars/2`, conservative) + `MAX_OUTPUT_TOKENS` (4096) → credits via
  `app_usage_credits`, 402 if balance short → stream w/ `max_completion_tokens`
  (OpenAI) / `max_tokens` (xAI) cap → after stream `app_record_usage` (real usage) →
  SSE `event: usage {tokens, costUsd, creditsCharged, balanceCredits}`. Provider-routed
  by `cg_models` (xAI cost from response `cost_in_usd_ticks`; OpenAI from token rates).
- `image`: modes `models` / `compose` (build prompt, billed) / `render` (generate).
  Render uses a **fixed-cost pre-auth gate** (image `price_usd` is known upfront →
  require `balance ≥` its credit cost before calling). Provider-routed by
  `cg_image_models`; xAI bills even on a moderation block (HTTP 400), OpenAI doesn't.
...
- **Trial is per-service**: chat_grok = 0 (top-up required); planlet keeps its own.
  Set in BOTH the function const `TRIAL_CREDITS` AND `app_pricing.trial_credits` (the
  two seed paths must agree — first to create the credits row wins).
...
- Supabase secrets (shared project): `XAI_API_KEY`, `OPENAI_API_KEY`, `GOOGLE_PLAY_SA_B64`.
...
that `completed` works. Currently **versionCode 25** is live on internal.
...
- **Never `supabase config push`** — shared project; it overwrites auth config.
  ...
  see auto-memory.)
- **DB migrations live in `android-planlet/supabase/migrations`** (shared), not here.
  Push from there. **Version-collision risk**: the planlet team may take the same
  `YYYYMMDDHHMMSS` prefix — check `supabase migration list` first; on a duplicate-key
  error rename your file to a free later timestamp and re-push.
- **Trial / signups can't be locked to the app.** A JWT is obtainable via Supabase's
  hosted Google OAuth by anyone (uses the server-side client secret); "Allow new
  signups" is project-global (shared with planlet). Cost is bounded by credits only —
  trial = 0 means a bare JWT costs nothing. A real app-only gate needs Play Integrity
  attestation in the functions (not built).
